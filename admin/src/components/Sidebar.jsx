...
const Sidebar = () => {
  ...
  const baseClass = 'flex items-center w-full gap-3 pl-3 py-3 hover:bg-gray-100'
  const isActiveClass = 'bg-[#e5f4ea] border-r-[5px] border-[#3cc489]'

  return (
    <div className='h-[91vh] w-64 absolute left-0 top-[60px] z-10 border-r bg-white border-gray-300 pt-3'>
      <div className='flex flex-col w-full gap-1'>
        <Link
          to="/add-product"
          onClick={() => setActiveItem('add-product')}
          className={`${baseClass} ${activeItem === 'add-product' ? isActiveClass : ''}`}
        >
        ...
        <Link
          ...
          className={`${baseClass} ${activeItem === 'product-list' ? isActiveClass : ''}`}
        >
        ...
        <Link
          ...
          className={`${baseClass} ${activeItem === 'orders' ? isActiveClass : ''}`}
        >
